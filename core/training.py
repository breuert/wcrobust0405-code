        ...
        for entry in csv.reader(file):
            ...
            if str(topic) == topic_tmp:
                ...
                try:
                    ...
                    if int(relevance) == 2:  # Grossman et al. convert relevance assessments to a binary scale
                        y.append(1)
                    else:
...
def train(train_feat, topic, n_feat, model_type='logreg-scikit'):
    ...
    if model_type == 'logreg-sofia':  # not recommended...
        model = sofia.Logreg(n_feat, sofia_ml='./../../sofia-ml')
        model.fit(train_feat+'tfidf_feat_'+str(topic))
