...
def _parse_trec(full_path_file, tmp, dir_raw_txt):
    ...
    if name not in skip_list:
        ...
        if name.endswith(".z"):
            ...
            shutil.copy(full_path_file, doc_path_tmp)
            doc_path_tmp_uncompr = tmp + name[:-2]
            os.system("gzip -d " + doc_path_tmp)

        if name.endswith(".gz"):

            doc_path_tmp = tmp + name
            shutil.copy(full_path_file, doc_path_tmp)
            doc_path_tmp_uncompr = tmp + name[:-3]
            os.system("gzip -d " + doc_path_tmp)

        markup = open(doc_path_tmp_uncompr, 'r', encoding="ISO-8859-1")
        try:
            text = markup.read()
        except:
...
def raw_text_from_times(times_data, extraction_dir, raw_text_dir):
    ...
    # path, subdirs, files = os.walk(times_data)
